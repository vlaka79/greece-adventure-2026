(function () {
  ...
  var photoLayer = null;
  var tripMap = null;
  var PATH_URL = "/trips/hawaii-2024/path.json";
  var AIS_URL = null;
  var ALBUM_URL = "/trips/hawaii-2024/album.json";
  var DRIVES_URL = "/trips/hawaii-2024/drives.json";
  var MILES_URL = "/trips/hawaii-2024/miles.json";
  ...
  function rebuild() {
    ...
    Promise.all(fetches).then(function (quad) {
      ...
      if (bounds.length) {
        tripMap.fitBounds(bounds, { padding: [28, 28], animate: false, maxZoom: 8 });
      } else {
        tripMap.setView([20.8, -157.0], 7);
      }
      setTimeout(function () { if (tripMap) tripMap.invalidateSize(); }, 200);
      setTimeout(function () { if (tripMap) tripMap.invalidateSize(); }, 800);
    }).catch(function (err) {
      console.warn(err);
      if (tripMap) tripMap.setView([20.8, -157.0], 7);
    });
  }
